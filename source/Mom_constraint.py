...
def get_Mom_diagnostic(solutions_over_time, t, R, N_r, r_is_logarithmic) :
    start = time.time()
    ...
    for i in range(num_times) :
        ...
        Mom_i = np.zeros((len(u),3)) # 3-vector now 
        # Mom_i   = np.zeros_like(u)
        ...
        for ix in range(num_ghosts, N-num_ghosts) :
            ...
            bar_gamma = np.einsum('ij,ij',bar_gamma_LL,bar_gamma_UU)
            ...
            # Get the Mom constraint eqn (14) of Baumgarte https://arxiv.org/abs/1211.6632
            Mom_i[0][ix] = em4phi*( 1/np.sqrt(bar_gamma) * 
                                   + 6.0*)
                           - eight_pi_G * S_i[0] # should have G here, too?
